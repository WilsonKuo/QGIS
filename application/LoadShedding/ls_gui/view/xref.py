...
import logging
import os, datetime
from model.xref import DEFAULT_HEADERS
from collections       import OrderedDict
from core.QtCompat import QtWidgets, QtGui, QtCore, Qt, Signal, Slot

logger = logging.getLogger(__name__)

class XREFTableView(QtWidgets.QTableView):
    def __init__(self, parent=None):
        super(XREFTableView, self).__init__(parent)
    ...
    def setModel(self, model):
        ...
        super(XREFTableView, self).setModel(model)
    ...
    def model(self):
        ...
        """
        model = super(XREFTableView, self).model()
    ...
    def _show_context_menu(self, position):
        ...
        feeders  = list([self.model().get_feeder(idx.row()) for idx in selected])
        # get the rtdb_point address of the lslockflag column
        address  = list([self.model().get_lockflag_addr(idx.row()) for idx in selected])
        # QMenu settings
        menu = QtWidgets.QMenu() 
        action1 = menu.addAction("LSLOCKFLAG: Enable") 
        action2 = menu.addAction("LSLOCKFLAG: Disable")
        actions = menu.exec_(self.mapToGlobal(position)) 
        if actions == action1: 
            answer = QtWidgets.QMessageBox.question(self, "Update Setting Table", f"Are you sure to set lockflag to <b>enable</b>?", QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No)
            if answer == QtWidgets.QMessageBox.Yes: 
                self.model().set_dasdb_lockflag(feeders, 0)
                self.model().set_rtdb_lockflag(address , 0)
            
        if actions == action2:
            answer = QtWidgets.QMessageBox.question(self, "Update Setting Table", f"Are you sure to set lockflag to <b>disable</b>?", QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No)
            if answer == QtWidgets.QMessageBox.Yes: 
                self.model().set_dasdb_lockflag(feeders, 1)
                self.model().set_rtdb_lockflag(address , 1)
