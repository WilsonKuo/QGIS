...
import logging
import datetime
from model.uf       import DEFAULT_HEADERS
from collections           import OrderedDict
from core.QtCompat import QtWidgets, QtGui, QtCore, Qt, Signal, Slot

logger = logging.getLogger(__name__)

class UFTableView(QtWidgets.QTableView):
    def __init__(self, parent=None):
        super(UFTableView, self).__init__(parent)
    ...
    def setModel(self, model):
        ...
        super(UFTableView, self).setModel(model)
    ...
    def model(self):
        ...
        """
        model = super(UFTableView, self).model()
    ...
    def _show_context_menu(self, position):
        ...
        indexes = list(set([self.model().get_ufid(idx.row()) for idx in selected]))
        address = list([self.model().get_lf_rtdb_address(idx.row()) for idx in selected])
        # QMenu settings
        menu = QtWidgets.QMenu() 
        action1 = menu.addAction("LOCKFLAG: Enable") 
        action2 = menu.addAction("LOCKFLAG: Disable")
        actions = menu.exec_(self.mapToGlobal(position)) 
        if actions == action1: 
            answer = QtWidgets.QMessageBox.question(self, "Update UF Table", f"Are you sure to set lockflag to <b>enable</b>? {indexes}", QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No)
            if answer == QtWidgets.QMessageBox.Yes: 
                # self.model().set_dasdb_lockflag(indexes, 0)
                self.model().set_rtdb_lockflag(address , 0)

        if actions == action2:
            answer = QtWidgets.QMessageBox.question(self, "Update UF Table", f"Are you sure to set lockflag to <b>disable</b>? {indexes}", QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No)
            if answer == QtWidgets.QMessageBox.Yes: 
                # self.model().set_dasdb_lockflag(indexes, 1)
